    ...
    <div>
      ...
      <div className="text-center pb-4 mb-4 border-b border-border">
        ...
        <p className="text-[13.5px] text-text-muted mb-4">
          {okCount}/{total} câu đúng · {pct >= 80 ? 'Xuất sắc! 🎉' : pct >= 60 ? 'Khá tốt 💪' : 'Cần ôn thêm 📖'}
        </p>

        {/* Stats row */}
        <div className="flex gap-2 justify-center mb-4">
          {[
            { label: 'Đúng', value: okCount, color: 'text-brand-green' },
            { label: 'Sai', value: wrongCount, color: 'text-brand-red' },
            { label: 'Tổng', value: total, color: 'text-text' },
          ].map(s => (
        ...
        <div className="flex gap-2 justify-center flex-wrap">
          ...
          >
            <RefreshCw size={13} /> Làm lại từ đầu
          </button>
          {wrongCount > 0 && (
            <button
            ...
            >
              <AlertCircle size={13} /> Chỉ câu sai ({wrongCount})
            </button>
          )}
        </div>
      </div>
      ...
      {wrongItems.length > 0 && (
        <div>
          <h4 className="text-[11px] font-bold text-text-faint uppercase tracking-wider mb-2">Các câu sai:</h4>
          {wrongItems.map((item, i) => (
            <div key={i} className="bg-page border border-border rounded px-3 py-2.5 mb-2 text-[12.5px]">
              ...
              <p className="text-brand-red mb-0.5">✗ Bạn chọn: {item.chosenText}</p>
              <p className="text-brand-green mb-1.5">✓ Đúng: {item.correctText}</p>
