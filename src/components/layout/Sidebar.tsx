  ...
  return (
    <aside className="w-[260px] bg-card border-r border-border flex flex-col flex-shrink-0 overflow-hidden">
      ...
      <div className="p-3 pb-2.5 border-b border-border flex-shrink-0">
        <p className="text-[10px] text-text-faint uppercase tracking-widest font-semibold mb-0.5">Chương 1</p>
        <p className="text-[12.5px] font-semibold text-text leading-snug mb-2">{chapter?.title}</p>
        <div className="flex items-center gap-2 text-[11px] text-text-faint">
          <span>{doneCount}/{lessons.length} bài</span>
            ...
            >
              <span
                className={`text-[10px] rounded px-1.5 py-0.5 font-semibold min-w-[19px] text-center flex-shrink-0 ${
                  isActive ? 'bg-brand-blue text-white' : 'bg-secondary text-text-faint'
                }`}
              >
                {i + 1}
              </span>
